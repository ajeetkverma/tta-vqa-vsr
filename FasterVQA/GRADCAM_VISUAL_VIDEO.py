...
class GradCAM:
    ...
    def generate(self, input_tensor):
        ...
        pooled_grads = torch.mean(self.gradients, dim=[0, 2, 3, 4])  # (C,)
        activations = self.activations.squeeze(0)  # (C, T, H, W)

        for i in range(activations.shape[0]):
            activations[i] *= pooled_grads[i]

        heatmap = torch.mean(activations, dim=0).cpu().numpy()  # (T, H, W)
        heatmap = np.maximum(heatmap, 0)
        for t in range(heatmap.shape[0]):
            heatmap[t] /= np.max(heatmap[t]) + 1e-8

        return heatmap  # shape: (T, H, W)
...
def main():
    ...
    for key, val_loader in val_loaders.items():
        for i, data in enumerate(tqdm(val_loader)):
            ...
            try:
                target_layer = model.fragments_backbone.layers[2].blocks[4]
            ...
            video_writer = cv2.VideoWriter(
                f"gradcam_outputs/{data['name'][0]}_gradcam.mp4",
            ...
            for t in range(heatmap.shape[0]):
                frame_tensor = input_clip['fragments'][0, :, t, :, :].cpu()  # C, H, W
                frame = frame_tensor.permute(1, 2, 0).numpy()
                frame = cv2.normalize(frame, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

                overlay = overlay_heatmap(heatmap[t], frame)
                video_writer.write(overlay)

            video_writer.release()
            print(f"✅ Grad-CAM video saved to: gradcam_outputs/{data['name'][0]}_gradcam.mp4")
