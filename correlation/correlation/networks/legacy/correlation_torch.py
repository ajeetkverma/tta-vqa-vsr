"""
This file contains the pytorch implementation of the correlation layer needed
for Flownet and LFNet. The implementation is using torch and all operations 
are tensor operations so it works on both cpu and gpu. Also this is pure python
so no need to install it. Just import where needed.

If running on CPU, use the C++ implementation in correlation_cpp wherever 
possible because that is atleast 2x faster.

If running on the gpu use the cuda implementation from correlation_cuda.
"""
